...
if [[ "$choice" == [Yy]* ]]; then
	for key in "${!VMS[@]}"; do
		...
		NETPARAMS="--net0 bridge=vmbr0,firewall=1"
		if [[ ${vmfields[3]} ]]; then
			MACADDR=${vmfields[3]}
		...
		echo "qm set $NEWGUESTID --ciuser $CIUSER --cipassword=$CIPASS --sshkey $SSHKEY --memory $MEMORY --ipconfig0 ip=dhcp $NETPARAMS"
		echo
	done
else
	echo -e "Aborted...\n"
...
if [[ "$choice" == [Yy]* ]]; then
	for key in "${!VMS[@]}"; do
		...
		IFS=',' read -r -a vmfields <<< "$vmdata"
		VMNAME=${fields[1]}
		echo "qm start $key ($VMNAME)"; sleep $qm_start_delay
	done
else
	exit 0
fi	
